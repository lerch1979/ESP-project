...
(async () => {
  try {
    ...
    check('groupValid: EMPTY shift is never placed with anyone', engine.groupValid([{ gender: 'male', shift: 'ejszaka', workplace: 'Audi' }, { gender: 'male', shift: null, workplace: 'Audi' }], m) === false);
    check('groupValid: two EMPTY shifts still incompatible', engine.groupValid([{ gender: 'male', shift: null, workplace: 'Audi' }, { gender: 'male', shift: null, workplace: 'Audi' }], m) === false);
    check('groupValid rejects MIXED WORKPLACE (same gender+shift)', engine.groupValid([{ gender: 'male', shift: 'delelott', workplace: 'Audi' }, { gender: 'male', shift: 'delelott', workplace: 'Mercedes' }], m) === false);
    check('groupValid allows same gender+shift+workplace', engine.groupValid([{ gender: 'male', shift: 'delelott', workplace: 'Audi' }, { gender: 'male', shift: 'delelott', workplace: 'Audi' }], m) === true);

    // ── deterministic scenarios on consolidateAccommodation (no random seed) ──
    const WP = 'Audi'; // shared workplace so same-gender+shift+workplace can consolidate
    // A) same everything: two half-full rooms merge → 1 freed.
    const twoRooms = [{ id: 'r1', beds: 2 }, { id: 'r2', beds: 2 }];
    const same = engine.consolidateAccommodation(twoRooms, [
      { id: 'a', gender: 'male', shift: 'delelott', workplace: WP, room_id: 'r1' },
      { id: 'b', gender: 'male', shift: 'delelott', workplace: WP, room_id: 'r2' },
    ], m);
    check('A) same-shift+workplace consolidates → 1 room freed, 1 move', same.valid && same.freedRooms.length === 1 && same.moves.length === 1);
    ...
    const cross = engine.consolidateAccommodation(twoRooms, [
      { id: 'a', gender: 'male', shift: 'delelott', workplace: WP, room_id: 'r1' },
      { id: 'b', gender: 'male', shift: 'ejszaka', workplace: WP, room_id: 'r2' },
    ], m);
    check('B) cross-shift is BLOCKED → 0 rooms freed, 0 moves', cross.valid && cross.freedRooms.length === 0 && cross.moves.length === 0);

    // B2) cross-WORKPLACE: same gender+shift but different employers — must NOT merge.
    const crossWp = engine.consolidateAccommodation(twoRooms, [
      { id: 'a', gender: 'male', shift: 'delelott', workplace: 'Audi', room_id: 'r1' },
      { id: 'b', gender: 'male', shift: 'delelott', workplace: 'Mercedes', room_id: 'r2' },
    ], m);
    check('B2) cross-WORKPLACE is BLOCKED → 0 rooms freed, 0 moves', crossWp.valid && crossWp.freedRooms.length === 0 && crossWp.moves.length === 0);

    // C) incomplete data (empty shift): flagged + never moved + room kept, while
    //    complete residents still consolidate around it.
    const threeRooms = [{ id: 'r1', beds: 2 }, { id: 'r2', beds: 2 }, { id: 'r3', beds: 2 }];
    const withEmpty = engine.consolidateAccommodation(threeRooms, [
      { id: 'a', gender: 'male', shift: 'delelott', workplace: WP, room_id: 'r1' },
      { id: 'b', gender: 'male', shift: 'delelott', workplace: WP, room_id: 'r2' },
      { id: 'x', gender: 'male', shift: null, workplace: WP, room_id: 'r3' }, // empty shift, alone in r3
    ], m);
    check('C) incomplete-data employee is FLAGGED', (withEmpty.flaggedIncomplete || []).includes('x'));
    check('C) incomplete-data employee is NEVER moved', !withEmpty.moves.some((mv) => mv.employee_id === 'x'));
    check('C) complete residents still consolidate (1 freed), incomplete room kept', withEmpty.valid && withEmpty.freedRooms.length === 1 && !withEmpty.freedRooms.includes('r3'));

    // C2) empty WORKPLACE is also flagged + never moved (missing workplace = incomplete).
    const withNoWp = engine.consolidateAccommodation(threeRooms, [
      { id: 'a', gender: 'male', shift: 'delelott', workplace: WP, room_id: 'r1' },
      { id: 'b', gender: 'male', shift: 'delelott', workplace: WP, room_id: 'r2' },
      { id: 'y', gender: 'male', shift: 'delelott', workplace: null, room_id: 'r3' }, // no workplace
    ], m);
    check('C2) empty-workplace employee is FLAGGED + never moved', (withNoWp.flaggedIncomplete || []).includes('y') && !withNoWp.moves.some((mv) => mv.employee_id === 'y'));
    ...
    check('incomplete-data employees were flagged for data entry', run.summary.flagged_incomplete_count >= 1);
    console.log(`      → run ${run.run_id}: ${run.total_moves} moves, frees ${run.freed_rooms} rooms / ${run.freed_beds} beds; ${run.summary.flagged_incomplete_count} incomplete flagged`);
    ...
    // No suggestion may target a flagged (incomplete-data) employee.
    const flaggedIds = new Set((run.summary.flagged_incomplete || []).map((f) => f.employee_id));
    check('NO incomplete-data employee appears in any move', suggestions.every((s) => !flaggedIds.has(s.entity_id)));
    ...
    let genderBad = 0, shiftBad = 0, capBad = 0, workplaceBad = 0;
    for (const [rid, members] of byRoom) {
      ...
      const placeable = members.filter((x) => engine.shiftBucket(x.shift) !== null && !!x.workplace);
      if (!engine.groupValid(placeable, m)) shiftBad++; // no two different KNOWN shifts share a room
      // INDEPENDENT workplace check: no two different workplaces share a room.
      const wps = new Set(placeable.map((x) => x.workplace));
      if (wps.size > 1) workplaceBad++;
    }
    ...
    check('NO mixed-WORKPLACE room in any touched site after applying ALL suggestions', workplaceBad === 0);
    ...
    for (const [, mem] of postByRoom) {
      const placeable = mem.filter((x) => engine.shiftBucket(x.shift) !== null && !!x.workplace);
      if (mem.length > mem[0].beds || mem.some((x) => x.gender !== mem[0].gender) || !engine.groupValid(placeable, m)) postBad++;
    }
