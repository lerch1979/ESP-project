...
async function teardown() {
  ...
  const stmts = [
    ...
    `DELETE FROM video_sequence_sends WHERE sequence_id IN (SELECT id FROM video_sequences WHERE name LIKE '${TAG} %')`,
    `DELETE FROM video_announcement_recipients WHERE announcement_id IN (SELECT id FROM video_announcements WHERE video_id IN (SELECT id FROM videos WHERE title LIKE '${TAG} %'))`,
    `DELETE FROM video_announcements WHERE video_id IN (SELECT id FROM videos WHERE title LIKE '${TAG} %')`,
    `DELETE FROM video_sequence_steps WHERE sequence_id IN (SELECT id FROM video_sequences WHERE name LIKE '${TAG} %')`,
    `DELETE FROM video_sequences WHERE name LIKE '${TAG} %'`,
    `DELETE FROM video_views WHERE video_id IN (SELECT id FROM videos WHERE title LIKE '${TAG} %')`,
    `DELETE FROM video_versions WHERE video_id IN (SELECT id FROM videos WHERE title LIKE '${TAG} %')`,
    `DELETE FROM videos WHERE title LIKE '${TAG} %'`,
