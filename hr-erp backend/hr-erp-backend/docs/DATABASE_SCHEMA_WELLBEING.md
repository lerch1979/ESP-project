...
17 tables across 3 modules supporting employee wellbeing tracking, EAP services, and cross-module integration.

| Module | Tables | Purpose |
|--------|--------|---------|
| Blue Colibri | 7 | Preventive wellbeing: pulse surveys, assessments, interventions, coaching, ML predictions |
| EAP | 6 | Employee Assistance Program: cases, sessions, providers, bookings |
...
```mermaid
erDiagram
    users ||--o{ blue_colibri_pulse_surveys : submits
    users ||--o{ blue_colibri_assessments : completes
    users ||--o{ blue_colibri_interventions : receives
    users ||--o{ blue_colibri_coaching_sessions : attends
    users ||--o{ blue_colibri_ml_predictions : predicted
    users ||--o{ eap_cases : opens
    users ||--o{ eap_provider_bookings : books
    users ||--o{ wellbeing_referrals : referred
    users ||--o{ wellbeing_notifications : receives
    users ||--o{ wellbeing_feedback : gives

    contractors ||--o{ blue_colibri_pulse_surveys : scopes
    contractors ||--o{ blue_colibri_team_metrics : aggregates
    contractors ||--o{ eap_cases : scopes
    contractors ||--o{ eap_usage_stats : tracks

    eap_service_categories ||--o{ eap_cases : categorizes
    eap_providers ||--o{ eap_cases : assigned_to
    eap_providers ||--o{ eap_sessions : conducts
    eap_providers ||--o{ eap_provider_bookings : receives
    eap_cases ||--o{ eap_sessions : contains
    eap_cases ||--o{ eap_provider_bookings : scheduled_for

    blue_colibri_interventions ||--o{ blue_colibri_coaching_sessions : triggers
```

## Tables

### Blue Colibri Module (Migration 058)

#### blue_colibri_questions
...
#### eap_cases
Employee cases. `issue_description` and `resolution_notes` encrypted via application-layer AES-256-CBC. Admin can see metadata but NOT case content. Supports anonymous mode.

#### eap_sessions
Session notes encrypted via pgcrypto (`pgp_sym_encrypt`). Only provider + case owner can access.

#### eap_provider_bookings
...
#### wellbeing_referrals
Cross-module referrals. Auto-expires after 30 days via trigger.
...
|-------|----------|-------------|
| questions | 2 | All read, admin write |
| pulse_surveys | 2 | Own data + admin read |
| assessments | 2 | Own data + admin read |
| interventions | 2 | Own data + admin full |
| coaching_sessions | 2 | Own + coach + admin |
| team_metrics | 2 | Manager/admin only |
| ml_predictions | 1 | Admin only |
| eap_categories | 2 | All read, admin write |
| eap_providers | 2 | Active read, admin full |
| eap_cases | 2 | Own + admin metadata |
| eap_sessions | 3 | Own + provider + admin |
| eap_bookings | 3 | Own + provider + admin |
| eap_usage_stats | 2 | Admin only |
| referrals | 2 | Own + admin/manager |
| notifications | 3 | Own read + own update + admin |
| audit_log | 4 | Admin read, system insert, no update, no delete |
| feedback | 2 | Own + admin read |
...
FROM blue_colibri_pulse_surveys
...
FROM blue_colibri_ml_predictions
...
| ID | Name | Description |
|----|------|-------------|
| 058 | blue_colibri_schema | 7 Blue Colibri tables + RLS + seed questions |
| 059 | eap_schema | 6 EAP tables + pgcrypto + RLS + seed providers |
