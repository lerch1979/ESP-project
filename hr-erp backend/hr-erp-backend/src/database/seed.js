...
async function seedDatabase() {
  try {
    ...
    // 1. Tenant-ok létrehozása (ON CONFLICT skip)
    ...
    const tenantResult = await query(`SELECT id, name, slug FROM tenants WHERE slug IN ('abc-kft', 'xyz-zrt') ORDER BY slug`);
    const tenant1Id = tenantResult.rows.find(t => t.slug === 'abc-kft').id;
    const tenant2Id = tenantResult.rows.find(t => t.slug === 'xyz-zrt').id;
    logger.info(`✓ Tenant-ok rendben (${tenantResult.rows.length} db)`);
    ...
    const userDefs = [
      // Szuperadmin
      { tenantId: tenant1Id, email: 'admin@hr-erp.com', firstName: 'Admin', lastName: 'User', role: 'superadmin' },
      // ABC Kft. felhasználók
      { tenantId: tenant1Id, email: 'kiss.janos@abc-kft.hu', firstName: 'Kiss', lastName: 'János', role: 'admin' },
      { tenantId: tenant1Id, email: 'nagy.eva@abc-kft.hu', firstName: 'Nagy', lastName: 'Éva', role: 'task_owner' },
      { tenantId: tenant1Id, email: 'toth.anna@abc-kft.hu', firstName: 'Tóth', lastName: 'Anna', role: 'user' },
      // XYZ Zrt. felhasználók
      { tenantId: tenant2Id, email: 'kovacs.peter@xyz-zrt.hu', firstName: 'Kovács', lastName: 'Péter', role: 'admin' },
      { tenantId: tenant2Id, email: 'szabo.maria@xyz-zrt.hu', firstName: 'Szabó', lastName: 'Mária', role: 'user' },
      // Külső alvállalkozók
      { tenantId: tenant1Id, email: 'vizvezetek@example.com', firstName: 'Vízvezeték', lastName: 'Kft.', role: 'contractor' },
      { tenantId: tenant1Id, email: 'it-support@example.com', firstName: 'IT', lastName: 'Support', role: 'contractor' },
      // Szállásolt munkavállalók (ABC Kft.)
      { tenantId: tenant1Id, email: 'horvath.gabor@employee.com', firstName: 'Horváth', lastName: 'Gábor', phone: '+36 30 123 4567', role: 'accommodated_employee' },
      { tenantId: tenant1Id, email: 'molnar.zsuzsanna@employee.com', firstName: 'Molnár', lastName: 'Zsuzsanna', phone: '+36 30 234 5678', role: 'accommodated_employee' },
      { tenantId: tenant1Id, email: 'varga.istvan@employee.com', firstName: 'Varga', lastName: 'István', phone: '+36 30 345 6789', role: 'accommodated_employee' },
      { tenantId: tenant1Id, email: 'farkas.katalin@employee.com', firstName: 'Farkas', lastName: 'Katalin', phone: '+36 30 456 7890', role: 'accommodated_employee' },
    ];
    ...
    for (const u of userDefs) {
      ...
      `, [u.tenantId, u.email, passwordHash, u.firstName, u.lastName, u.phone || null]);
      ...
      if (roles[u.role]) {
        ...
        `, [userId, roles[u.role], u.tenantId]);
      }
    }
    ...
    const categoriesResult = await query(
      'SELECT id, slug FROM ticket_categories WHERE tenant_id = $1', [tenant1Id]
    );
    const categories = {};
    ...
    for (const t of ticketDefs) {
      ...
      `, [
        tenant1Id, t.number, t.title, t.description,
