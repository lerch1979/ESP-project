...
- `migrations/` (under backend) — numbered SQL, latest is **133** (2026-07-05, consolidation-engine v2: accommodation roles/lock + workplace binding + stability — **SANDBOX ONLY, not on prod**). Prod's latest is **131** (`scheduled_report_run_output`); **132 (consolidation v1) + 133 (consolidation v2) are applied to the sandbox DB only.** ⚠️ Runner blocked at `093` on the **dev DB only** (6 users); fresh DBs (CI/test/sandbox) migrate cleanly through the latest. Newer migrations (118+) applied to dev AND prod via `psql`; see tech-debt.
...
**Room Consolidation Suggestion Engine — v1 + v2 BUILT & TESTED in the sandbox (2026-07-05); NOT deployed:**
- **Status:** engine is built (v1 within-accommodation, v2 accommodation-level strategy: roles/lock/workplace-binding/stability/cross-moves), proven by a 40-check idempotent test suite, and demoed end-to-end in the admin UI on the sandbox. Deploying = apply mig 132+133 to prod + populate real room/shift/role/workplace data (a separate go decision). The v2 detail is in the 2026-07-05 RECENT WORK row above; strategy semantics are documented in `consolidationEngine.service.js`.
