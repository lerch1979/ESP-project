...
- `migrations/` (under backend) — numbered SQL, latest is **134** (2026-07-05, consolidation-engine v3: plan approval → move-task lifecycle — **SANDBOX ONLY, not on prod**). Prod's latest is **131** (`scheduled_report_run_output`); **132 (consolidation v1) + 133 (v2 strategy) + 134 (v3 lifecycle) are applied to the sandbox DB only.** ⚠️ Runner blocked at `093` on the **dev DB only** (6 users); fresh DBs (CI/test/sandbox) migrate cleanly through the latest. Newer migrations (118+) applied to dev AND prod via `psql`; see tech-debt.
...
| Date | Commit | Summary |
|---|---|---|
| 2026-07-05 | _sandbox_ | **Consolidation v3 — approval → MOVE TASK lifecycle (SANDBOX ONLY — not deployed).** Approval no longer silently rewrites room assignments: it creates ONE move ticket per plan (reuses the ticket system, category `moving`/"Költözés", assignee + due date chosen at approval, body = human-readable move list) and defers the actual room reassignment until the physical move is **confirmed done** — instruct → execute → confirm. Rooms (and thus occupancy snapshots + billing) change only at confirm; the stability clock also starts at confirm. mig 134 (`consolidation_plans` lifecycle table; `agent_suggestions.status` +`approved`/`skipped`/`cancelled`). Engine `approvePlan`/`confirmMove`/`cancelPlan`/`getPlans` (replaced `applyGroup`): confirm applies exactly the CHECKED moves atomically with per-move done/skip checkboxes (unchecked → `skipped`+reason → plan `partially_moved`), re-validates against current state (stale plan → **conflict surfaced**, not silent), rooms/history written only then; cancel closes the ticket with no changes. UI: lifecycle chips (Javasolt → Jóváhagyva[ticket#/felelős/határidő] → Beköltöztetve/Részben/Visszavonva) + approve/confirm/cancel dialogs. Tests 60/60 (idempotent). Verified in-browser: approve → ticket #13 (0 room changes) → partial confirm (7 applied / 1 skipped w/ reason) → plan partially_moved, ticket completed. **mig 134 NOT applied to prod.** |
| 2026-07-05 | _sandbox_ | **Room Consolidation Engine v2 — accommodation-level strategy (SANDBOX ONLY — not deployed).** mig 133 (`accommodations.consolidation_role` core/buffer/normal/phase_out + `consolidation_locked`; `accommodation_workplaces` binding table; `consolidation_config.stability_days`+`weight_drain`). New HARD constraints proven on every suggestion: **workplace binding** (target accommodation must admit the employee's workplace; empty list=unrestricted), **lock** (locked site never touched), **stability** (60-day re-suggest cooldown from `entity_status_history`). Roles drive drain/fill order (buffer/phase_out drain first, core fills first). **Cross-accommodation moves** unlocked (v1 was same-accommodation only). Pure exported planner `planConsolidation` — two passes valid-by-construction (within-consolidation pinning frozen/locked, then cross-drain of buffer/phase_out into core-first/normal). **Plans** = accommodations linked by cross-moves (union-find), atomic apply per plan. `entity_status_history` records room + (cross) accommodation changes. Apply-time validator fixed to check only rooms a plan *composes*. Test suite 40/40 (pure-planner unit tests + full-run constraint proofs + apply/stability e2e, idempotent). Seed has a deterministic role cast (core/buffer/phase_out/locked) + workplace mappings. UI: role/lock/workplace editable on the accommodation admin; consolidation page groups by plan with role badges + cross-move marking (fixed `run.run_id`→`run.id` apply bug). Also fixed a pre-existing sandbox bug (`accommodations.type='worker_hostel'` not in `VALID_TYPES` → edit-save 400; → `dormitory`). Verified in-browser: run → approve cross-plan → **buffer fully drained into the workplace-bound core, phase_out into a normal**. **mig 133 NOT applied to prod.** |
| 2026-07-05 | _sandbox_ | **Sandbox blank-page fix + one-click Desktop launcher (SANDBOX ONLY).** The admin SPA rendered blank at `localhost:5173`: `src/i18n/index.js` imported the bundled locale JSON from **`public/locales/`**, which **Vite 5 refuses to import** (dev returns 503 for `?import`) → the i18n module threw at eval → `main.jsx` never mounted React. Fixed by `git mv public/locales → src/i18n/locales` + repointing the 5 imports (bundles cleanly for dev + build; nothing else referenced the old path — the resident i18n guard uses the *mobile* app's locales). Also built a terminal-free launcher: `scripts/sandbox/{start,stop}.sh` + double-clickable Desktop `HR-ERP Sandbox.command` / `HR-ERP Sandbox – Stop.command` (start = PATH fix, DB-bootstrap-if-missing, nohup both servers, poll `/health`+SPA, open Chrome; stop = kill by PID+port, DB untouched). Verified in-browser end-to-end: launcher → login → **Konszolidáció engine runs (33 moves / 7 rooms / 15 beds / 4 sites)**. Prod untouched, no deploy. |
...
**Room Consolidation Suggestion Engine — v1 + v2 + v3 BUILT & TESTED in the sandbox (2026-07-05); NOT deployed:**
- **Status:** engine is built (v1 within-accommodation, v2 accommodation-level strategy: roles/lock/workplace-binding/stability/cross-moves, v3 approval→move-task lifecycle), proven by a **60-check** idempotent test suite, and demoed end-to-end in the admin UI on the sandbox. **v3 lifecycle:** approving a plan creates a "Költözés" move ticket (assignee + due date) and changes NO room assignments; the actual room reassignment (+ stability clock) happens only when the physical move is **confirmed done** (per-move done/skip checkboxes → applied/partially_moved), or the plan is cancelled — so occupancy snapshots + billing always reflect reality. Deploying = apply mig 132+133+134 to prod + populate real room/shift/role/workplace data (a separate go decision). Detail in the 2026-07-05 RECENT WORK rows above; semantics documented in `consolidationEngine.service.js`.
...
- **Sandbox for building the engine (2026-07-05, local only):** `hr_erp_sandbox` DB + synthetic seed (`docs/SANDBOX.md`). `npm run sandbox:reset` = drop + migrate-from-scratch (fresh DB sidesteps the 093 block) + seed 15 accommodations / 95 rooms / 300 employees (70% room-assigned, day/night + mixed-gender edge cases) + 4 test logins (`*@sandbox.local` / `sandbox123`). Run backend via `npm run dev:sandbox`. Seed guards against non-sandbox DBs; prod/dev untouched. Build + test the consolidation engine here. **One-click launch (no terminal):** double-click **`HR-ERP Sandbox.command`** on the Desktop (starts backend :3001 + admin :5173 detached, waits for both, opens Chrome to `localhost:5173`); **`HR-ERP Sandbox – Stop.command`** stops them. Logic in `scripts/sandbox/{start,stop}.sh`; runtime PIDs/logs in `.sandbox-run/` (gitignored). Known Vite gotcha fixed 2026-07-05: locale JSON must live under `src/` not `public/` (Vite 5 won't import `public/` assets → 503 → blank page).
