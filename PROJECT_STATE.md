...
| Date | Commit | Summary |
|---|---|---|
| 2026-07-19 | `43435abc` _main→prod_ | **Billing Phase 1b: per-bed formula + compensation pass-through + lekötetlen.** mig 141: `client_night_rates` per_bed_night basis (rate_used/rate_empty/occupancy_floor_pct/contracted_beds); `accommodation_billings.compensation_amount`. Engine: contracted bed block billed every valid day (guarantee), `full=max(occupied, ceil(cap×floor))`, `reduced=max(0,cap−full)`, `net/day=full×rate_used+reduced×rate_empty`; cap = contracted_beds else physical beds; degenerates to per-occupied-bed. Compensation → worker's **megbízó** as a separate line (statuses issued/notified/partial_paid/escalated — **disputed excluded**; attribution=issued_date), OUT of housing margin; unresolvable comps surfaced. Profit dashboard: lekötetlen (physical−committed beds), empty bed-nights, kártérítés columns. Admin rate form + profit columns. Verified: worked examples (95→340000, Autoliv 60@90%→198000), unit 17 / jest 6 / integration 12 / regression 14. **Deployed + live-verified** (mig 141 via psql; new SQL paths clean on prod). Bills real numbers once beds+megbízók+per-bed rates populated. Phase 2 = progressive tiers. |
| 2026-07-19 | `b964a786` _main→prod_ | **Full billing model settled → Phase 1a: contractor_roles + exclusivity.** Owner reversed "Option A" (accommodation-derived client): `current_contractor_id` = **SZÁLLÁSADÓ** (landlord, COST), billing client = **per-employee MEGBÍZÓ** (`billing_client_id`, REVENUE). mig 140 `contractor_roles` junction (multi-tag: megbízó/szállásadó/alvállalkozó), backfilled from actual usage (not `contractors.type`). **Rule: megbízó ⊕ szállásadó mutually exclusive** (revenue vs cost), alvállalkozó independent — enforced in create + `PUT /contractors/:id/roles` + UI auto-clear. Roles in list/detail + `?role=` filter; role chips/toggle editor on Contractors; **"Ingatlan tulajdonos"→"Szállásadó"**, Employees **"Számlázási ügyfél"→"Megbízó"** (pickers role-filtered). Sandbox 18/18. **Prod fix:** Házi Anikó (Bük landlord) was mis-tagged megbízó via an artifact rate → removed role + deleted the rate → szállásadó-only, 0 conflicts (0 financial impact; billings were 0.00). **⚠ Gaps for owner:** Bük has no megbízó (31 workers); Barcza Gyuláné likely same-class mislabel (Sarród I.). **Next: Phase 1b** = per-bed formula (rate_used/rate_empty/occupancy_floor_pct/contracted_beds) + compensation→invoice line. |
